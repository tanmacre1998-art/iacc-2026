...
En `membrete/` está el papel membreteado oficial del Programa Académico de Matemáticas
(plantilla `MATEMATICAS_FCMN.dot`) reproducido en LaTeX, con la marca del encuentro integrada:

- `membrete.sty`: cabecera oficial (escudo, facultad, programa) y pie oficial (PBX, dirección,
  acreditación, línea de atención, correo) en Cambria; a la derecha de la cabecera, el isologo
  del encuentro (dibujado en TikZ, vectorial) y el sello de los 15 años de ITENUA. Incluye los
  comandos de carta.
...
```bash
lualatex carta-invitacion.tex
```

(Se compila con LuaLaTeX, no con pdflatex, porque usa la tipografía Cambria del sistema.
La primera vez compílalo dos veces.)
